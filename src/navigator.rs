            ...
            Action::DeleteStory { epic_id, story_id } => {
                if (self.prompts.delete_story)() {
                    self.db
                        ...
                        .context("could not delete story!")?;
                }
            }
            Action::Exit => {
                self.pages.clear();
